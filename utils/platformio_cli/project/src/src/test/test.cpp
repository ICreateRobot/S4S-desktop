 ...
 * @LastEditTime : 2026-05-14 11:59:54
...
static void timer_callback(zst_timer_t *timer)
{
    ...
    // auto addr = if_i2c_internal_handle.scan();
    // for (auto &it : addr)
    // {
    //     sprintf(display_buffer, "%x, ", it);
    //     hw_esp_oled.print(0 + pos*32, 0, display_buffer);
    //     pos++;
    // }

    sprintf(display_buffer, "%d", hw_ai_camera.tag_id(1));
    hw_esp_oled.print(0, 0, display_buffer);
}
