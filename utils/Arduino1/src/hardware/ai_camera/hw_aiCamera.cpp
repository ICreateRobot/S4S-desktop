 ...
 * @LastEditTime : 2026-05-08 10:05:28
...
int hw_aiCamera_c::tag_rotation(int id)
{
    ...
    int     ret_error   = 0;
    uint8_t id_data[12] = {0};
    ret_error           = this->readReg(this->AICAMERA_ADDR, APRILTAG_REG + id + 12, id_data, 12);
    if (ret_error != 0) { return ret_error; }
...
{
    DATA_CHECK(region, REGION_UP, REGION_DOWN);
...
// 识别出指定对象了吗？
int hw_aiCamera_c::detected(int object_class, int id)
{
...
int hw_aiCamera_c::face_position(int select, int id)
{
    ...
    DATA_CHECK(select, 0, 3);
    int     ret_error         = 0;
    uint8_t position_data[12] = {0};
    ret_error = this->readReg(this->AICAMERA_ADDR, FACE_ATTRIBUTE_REG + id, position_data, 12);
    if (ret_error != 0) { return ret_error; }
    return (position_data[select * 2] << 8) | position_data[select * 2 + 1];
}
